...
describe('template assistant service', () => {
  ...
  it('显式 tableApiPreset override 会覆盖默认 preset', async () => {
    ...
    expect(mockCallAIWithPreset).toHaveBeenCalledWith(expect.any(Array), 'assistant-preset', undefined, undefined, { needsJsonFormat: true, sessionNamespace: 'template-assistant' });
  });
